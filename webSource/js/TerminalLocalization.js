...
var TerminalLocalization = function (TERMINAL) {
    ...
    this._LOCALIZATION_BASE = {
        ...
        26: {
            en: "Please, enter watch name. Use /help for more information.",
            ru: "Введите цель слежения. Введите /help чтобы получить больше информации."
        },
        27: {
        ...
        44: {
            en: "No connection established. Try /connect first.",
            ru: "Соединение отсутствует. Установите его, используя /connect"
        },
        45: {
        ...
        50: {
            en: "<h1 class=\"center\">WebTerminal usage tips</h1>" +
                ...
                "<p>Start by typing something like \"set test = 12\". While entering " +
                ...
                "you need. After normal command execution try to execute \"write test\". You will notice available autocompletion variants for command \"write\" and for your variable \"test\". " +
            ...
            ru: "<h1 class=\"center\">Советы по использованию веб-терминала</h1>" +
                ...
                "<p>Начните с ввода чего-то похожего на \"set test = 12\". Пока вы вводите команду set, можно заметить " +
                ...
                "требуемого направления. После нормального выполнения команды попробуйте выполнить \"write test\". Будет заметно, что варианты автодополнения будут доступны не только для \"write\", но и для \"test\". " +
        ...
        51: {
            en: "<p>As you may have noticed, there's a set of client-side commands you can " +
                ...
                "rules of passing arguments in examples: </p><ul><li>/command argument1 argument2 ...</li>" +
                "<li>/command \"argument1 with spaces\" argument_2 \"\" it_was_argument_3 ...</li><li>argument №1 /command</li>" +
                "<li>a r g u \"m\" e n t 1 /command \"a r g u m e n t 2\" argument3 ...</li></ul>" +
                "<p>You can play with arguments by using /echo command. Try to pass something " +
            ...
            ru: "<p>Как вы могли заметить, существует набор команд, которые можно выполнять " +
                ...
                "</p><ul><li>/команда аргумент1 аргумент2 ...</li>" +
                "<li>/команда \"аргумент1 с пробелами\" аргумент_2 \"\" Это_был_третий_аргумент ...</li><li>аргумент №1 /команда</li>" +
                "<li>а р г у \"м\" е н т 1 /команда \"а р г у м е н т 2\" аргумент3 ...</li></ul>" +
                "<p>Можно более детально изучить работу парсера аргументов, исполнив команду /echo. " +
        ...
        52: {
            en: "<p>Let's explore a little more about autocompletion. For example, you wishes to autocomplete " +
                "your current classes in namespace. Because of large base of variants available, this implemented by /autocomplete</span> command. By calling this command you will cause " +
                ...
                "wish to regenerate this file, then execute this command with \"new\" argument: \"/autocomplete new\". </p>",
            ru: "<p>Теперь стоит добавить еще кое-что про автодополнение. Допустим, вам потребовалось дополнять классы " +
                "и методы в текущем пространстве имён. Из-за большой базы всех этих слов, такое достигается за счёт выполнения команды /autocomplete</span>. Вызывая эту команду, вы заставите сервер сгенерировать " +
                "файлы с данными, которые будут подгружены, или подгрузить уже существующие. Первое выполнение команды может " +
                "стать затратным, так как будет сгенерирован большой файл системных классов. Он генерируется единожды и удаляется только в случае команды /reset. Данная операция может занять 15-20 секунд, это " +
                ...
                "Последующие вызовы команды будут просто подгружать уже сгенерированные файлы. Если словарь для текущего пространства имён требуется сгенерировать заново, выполните \"/autocomplete new\". </p>"
        },
        53: {
            en: "<p>Terminal application has different settings you can change. Simply call /settings command and you will get a panel with list of options. To close " +
            ...
            ru: "<p>Приложение терминала имеет множество настроек. Просто выполните /settings и вы увидите панель с настройками. Чтобы закрыть панель, нажмите " +
        ...
        54: {
            en: "<p>Another thing storing in localStorage is terminal language and command history. Terminal " +
                ...
                "autocompletion. After /save command call, your settings, language and " +
                "history will be saved. Anytime you want it is possible to restore this settings - just call /load command. /reset command will " +
            ...
            ru: "<p>Так же успешно, как и настройки, в локальное хранилище попадает словарь, история и определения. Словарь " +
                ...
                "После вызова команды /save всё вышеперечисленное будет копировано в хранилище. " +
                "В любой другой момент можно загрузить это, выполнив команду /load. /reset сотрёт всё, вернув терминал в исходное состояние.</p>"
        },
        55: {
            en: "<p>There are more features you can obtain with client-side commands.</p><p>First, you can use /sql command to enter and exit simple sql-mode. Easiest then ever.</p><p>" +
                "Next, it is easy to define " +
                "any big command by using /define command. Try to execute something like </p><p>##class(%Library.File).Exists( /define ?f(</p><p> and sure how it works by " +
                "executing </p><p>write ?f(\"C:\")</p><p>Do not forget about autocomplete! " +
                "If you didn't, use /autocomplete option to load classes and examine how it " +
                "helps. Autocompletion have to help you enter \"%Library.File\" class and it's method \"Exists\".</p>" +
                "<p>Another cool thing is watching " +
                "(or tailing). You can call /watch or /tail command with one argument - " +
                "file path or global name. Let's imagine that you have file C:\\temp\\log.txt or global ^myLog. Then you can call /tail ^myLog or /watch C:\\temp\\log.txt and look for their's changes.</p>",
            ru: "<p>Теперь стоит рассмотреть, на что способны клиентские команды.</p><p>Для начала, следует попробовать /sql режим. Он позволит быстро выполнять SQL-запросы.</p><p>" +
                "Затем, можно определить любое сокращение, используя /define. Попробуйте выполнить </p><p>##class(%Library.File).Exists( /define ?f(</p><p> и убедитесь в том, как это работает,  " +
                "выполнив </p><p>write ?f(\"C:\")</p><p>Не забывайте про автодополнение! " +
                "Выполните /autocomplete, чтобы загрузить базу классов и их свойств. " +
                ...
                "Вызовом /watch или /tail с аргументом глобала или файла можно начать наблюдать за изменениями в них. " +
                "Давайте представим, что у вас есть файл C:\\temp\\log.txt или глобал ^myLog. Выполните /tail ^myLog или /watch C:\\temp\\log.txt и попробуйте их изменить. Стоит заметить, что для глобала это " +
                "действительно только в случае добавления измерений.</p>"
        },
        56: {
            en: "<p>And finally try to use /favorite command: write a piece of " +
                "code or whole program, test it and then join /favorite 1 to the end " +
                "of command line. This will remember you command to slot 1. Then simply call /favorite 1 (with one argument) and use use saved code again.</p><p>And that's the end of out tutorial! Do not forget " +
                "to /save your history and favorites, or just use \"autosave\" option " +
                "available in settings.</p>",
            ru: "<p>Ну и в завершение попробуйте /favorite: напишите часть кода или целую программу, " +
                "после чего добавьте в конец /favorite 1. Это запомнит команду (программу) в первый слот. Потом простым вызовом /favorite 1 (с одним аргументом) можно будет снова применить код.</p><p>И на этом всё! Не забывайте сохранять (/save) вашу историю или просто поставьте галочку напротив \"автосохранения\" в " +
                "настройках.</p>"
        },
        57: {
            en: "<p>Enjoy!</p>",
